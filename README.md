...
帐篷仓没有固定货架，用的是**俯视平面图**：地面切成 4排×8格 的粗网格（行1靠门，门在右下角），「划区域」时逐个点格子再确认——格子**可以不连贯、不规则**，选出来的一组格子就是一个「区域」（= 地上的一堆货，同区同色）。区域当箱子用：拖东西进去、搜索闪烁定位、点开看内容全是同一套机制；堆变大变小可以「改范围」（加减格子，货不动），散了就「解散」（货退回未归位）。
